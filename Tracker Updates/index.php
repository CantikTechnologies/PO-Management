...
// Check and fix any records without status_of_action field
try {
    $checkStmt = $pdo->query("SELECT COUNT(*) as count FROM finance_tasks WHERE status_of_action IS NULL OR status_of_action = ''");
    $nullCount = $checkStmt->fetch(PDO::FETCH_ASSOC)['count'];
    
    if ($nullCount > 0) {
        // Update any records with NULL or empty status_of_action to 'Pending'
        $updateStmt = $pdo->prepare("UPDATE finance_tasks SET status_of_action = 'Pending' WHERE status_of_action IS NULL OR status_of_action = ''");
        $updateStmt->execute();
    }
} catch(PDOException $e) {
...
// Calculate statistics with proper error checking
$total_trackers = count($trackers);
$completed_trackers = count(array_filter($trackers, fn($tracker) => isset($tracker['status_of_action']) && $tracker['status_of_action'] === 'Completed'));
$pending_trackers = count(array_filter($trackers, fn($tracker) => isset($tracker['status_of_action']) && $tracker['status_of_action'] === 'Pending'));
$in_progress_trackers = count(array_filter($trackers, fn($tracker) => isset($tracker['status_of_action']) && $tracker['status_of_action'] === 'In Progress'));
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <script src="tracker_updates_new.js?v=1"></script>
</body>
</html>