...
try {
    ...
    while($r = $res->fetch_assoc()){
        ...
        $r['net_payable'] = (float)$r['net_payble']; // Map database column to JavaScript field name
    ...
    echo "<h3>Data that JavaScript will receive:</h3>";
    echo "<table border='1' style='border-collapse: collapse; width: 100%;'>";
    echo "<tr><th>ID</th><th>Vendor Inv Number</th><th>Vendor Inv Value</th><th>TDS Ded</th><th>Net Payable</th><th>Payment Value</th><th>Pending Payment</th></tr>";
    ...
    foreach ($rows as $row) {
        ...
        echo "<td>₹" . number_format($row['net_payable'], 2) . "</td>";
    ...
    // Test JSON output
    echo "<h3>JSON Output (first entry):</h3>";
    if (count($rows) > 0) {
        echo "<pre>" . json_encode($rows[0], JSON_PRETTY_PRINT) . "</pre>";
    }
...
echo "<hr>";
echo "<p><a href='index.php'>← Back to Outsourcing Page (Should now work correctly)</a></p>";
?>
