          ...
          <table>
            ...
            <tbody>
              ...
              if ($search !== '' || $project_filter !== '') {
                ...
                if ($project_filter !== '') {
                  $where_conditions[] = "vpo.project_description = ?";
                ...
                if ($search !== '') {
                  $like = '%' . $search . '%';
                  $where_conditions[] = "(vpo.po_number LIKE ? OR vpo.project_description LIKE ? OR vpo.vendor_name LIKE ? OR vpo.cost_center LIKE ? OR vpo.po_status LIKE ?)";
                ...
                $stmt = $conn->prepare(
                  "SELECT vpo.*, po.sow_number, po.start_date, po.end_date, po.po_date, po.billing_frequency, po.target_gm, po.remarks
                   FROM v_purchase_orders vpo
                   LEFT JOIN purchase_orders po ON po.po_id = vpo.po_id
                   WHERE $where_clause
                   ORDER BY vpo.po_id DESC"
                );
              ...
              if ($res && $res->num_rows > 0) {
                while ($row = $res->fetch_assoc()) {
                  ...
                  echo '<td>'.$fmt($row['target_gm'] ?? '').'</td>';
                  echo '<td>'.$fmt($row['pending_amount_in_po'] ?? '').'</td>';
