        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
          <div>
            ...
            <select name="project" id="project_filter" onchange="this.form.submit()">
              ...
              <?php
              $projects = $conn->query("SELECT DISTINCT po.project_description 
                                        FROM outsourcing_details outd 
                                        JOIN purchase_orders po ON po.po_id = outd.po_id 
                                        WHERE po.project_description IS NOT NULL AND po.project_description != '' 
                                        ORDER BY po.project_description");
          ...
          <table>
            ...
            <tbody>
              ...
              if ($search !== '' || $project_filter !== '') {
                ...
                if ($search !== '') {
                  $like = '%' . $search . '%';
                  $where_conditions[] = "(po.po_number LIKE ? OR outd.vendor_invoice_no LIKE ?)";
                  $params = array_merge($params, [$like, $like]);
                  $param_types .= 'ss';
                }

                $where_clause = implode(' AND ', $where_conditions);
                $stmt = $conn->prepare("SELECT outd.*, po.po_number, po.cost_center, po.vendor_name, po.project_description, po.po_value, po.pending_amount_in_po, po.po_date, inv.payment_receipt_date 
                                        FROM outsourcing_details outd 
                                        JOIN purchase_orders po ON po.po_id = outd.po_id 
                                        LEFT JOIN invoices inv ON inv.po_id = outd.po_id AND inv.vendor_invoice_no = outd.vendor_invoice_no
                                        WHERE $where_clause 
                                        ORDER BY outd.outsourcing_id DESC");
                $stmt->bind_param($param_types, ...$params);
                $stmt->execute();
                $res = $stmt->get_result();
              } else {
                $res = $conn->query("SELECT outd.*, po.po_number, po.cost_center, po.vendor_name, po.project_description, po.po_value, po.pending_amount_in_po, po.po_date, inv.payment_receipt_date 
                                     FROM outsourcing_details outd 
                                     JOIN purchase_orders po ON po.po_id = outd.po_id 
                                     LEFT JOIN invoices inv ON inv.po_id = outd.po_id AND inv.vendor_invoice_no = outd.vendor_invoice_no
                                     ORDER BY outd.outsourcing_id DESC");
              ...
              if ($res && $res->num_rows > 0) {
                while ($r = $res->fetch_assoc()) {
                  ...
                  echo '<td>' . $fmt($r['cantik_po_date']) . '</td>';
                  echo '<td>' . $fmt($r['po_value']) . '</td>';
                  echo '<td>' . $fmt($r['pending_amount_in_po']) . '</td>';
                                     // --- FIXED Vendor Invoice Frequency ---
                   $vendor_frequency = $r['vendor_invoice_frequency'] ?? $r['invoice_frequency'] ?? $r['billing_frequency'] ?? '';
                  ...
                  echo '<td>' . $fmt($r['vendor_invoice_date']) . '</td>';
                  echo '<td>' . $fmt($r['vendor_invoice_value']) . '</td>';
                  echo '<td>' . $fmt($r['tds_ded']) . '</td>';
                  echo '<td>' . $fmt($r['net_payable']) . '</td>';

                  // --- FIXED Payment Status from NTT ---
                  if (!empty($r['payment_status'])) {
                    $payment_status = $r['payment_status'];
                  } elseif (!empty($r['payment_date'])) {
                    $payment_status = (new DateTime($r['payment_date']))->format('d-M-y');
                  } elseif (!empty($r['payment_receipt_date'])) {
                    $payment_status = (new DateTime($r['payment_receipt_date']))->format('d-M-y');
                  } else {
                  ...
                  if (!empty($r['payment_date'])) {
                    $formatted_date = (new DateTime($r['payment_date']))->format('d-M-y');
                  } elseif (!empty($r['payment_receipt_date'])) {
                    $formatted_date = (new DateTime($r['payment_receipt_date']))->format('d-M-y');
                  } else {
                    $formatted_date = '-';
                  }
                  echo '<td>' . $fmt($formatted_date) . '</td>';

                  echo '<td>' . $fmt($r['pending_payment']) . '</td>';
                  echo '<td>' . $fmt($r['remarks']) . '</td>';
                  echo '<td class="actions">
                          <a href="edit.php?id=' . htmlspecialchars($r['outsourcing_id']) . '" class="btn muted">Edit</a>
                          <a href="delete.php?id=' . htmlspecialchars($r['outsourcing_id']) . '" class="btn danger" onclick=\'return confirm("Are you sure you want to delete this record? This action cannot be undone.")\'>Delete</a>
                        </td>';
                  echo '</tr>';
                }
              } else {
